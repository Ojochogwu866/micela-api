...
export const login = async (req: express.Request, res: express.Response) => {
	try {
		...
		if (!isSubAccount) {
			return res
				.status(401)
				.json({ error: 'Email is not registered as a sub-account' });
		}
		...
		user.authentication.sessionToken = hashPassword(
			sessionToken,
			user._id.toString()
		);
		await user.save();
...
export const updateGithubUsername = async (
	...
	try {
		...
		if (!id) {
			return res
				.status(400)
				.json({ error: 'User ID and GitHub username are required' });
		}
